...
public class Jigsaw {
    ...
    private Vector<JigsawNode> solutionPath;// 解路径
                                            // ：用以保存从起始状态到达目标状态的移动路径中的每一个状态节点
     ...
     * 若搜索成功，则输出初始状态beginJnode，目标状态endJNode，已访问节点数searchedNodesNum，
     * 路径深度nodeDepth和解路径solutionPath。
     * 
     * @param pw
    ...
    private Vector<JigsawNode> findFollowJNodes(JigsawNode jNode) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (tempJNode.move(i)
                    && !this.closeList.contains(tempJNode)
    ...
    public boolean BFSearch() throws IOException {
        ...
        // (1)将起始节点放入openList中
        this.openList.insertElementAt(this.beginJNode, 0);
    ...
    /**
     * （Demo+实验二）启发式搜索。访问节点数大于25000个则认为搜索失败。 函数结束后：isCompleted记录了求解完成状态；
    ...
    private void estimateValue(JigsawNode jNode) {
        ...
        for (int index = 1; index < dimension * dimension; index++) {
            if (jNode.getNodesState()[index] + 1 != jNode.getNodesState()[index
                    + 1]) {
                s++;
            }
        }
        ...
        for (int i = 1; i <= dimension * dimension; i++) {
            ...
            if (jNode.getNodesState()[i] != i
                ...
                int tarRow = (jNode.getNodesState()[i] - 1) / dimension;
                int tarCol = (jNode.getNodesState()[i] - 1) % dimension;
                int curRow = (i - 1) / dimension;
                int curCol = (i - 1) % dimension;
                // 采用曼哈顿距离
                totalDistance += (int) (Math.pow(Math.sqrt(Math.abs(tarRow
                        - curRow) * 1.00), 2.00)
                        + Math.pow(Math.sqrt(Math.abs(tarCol - curCol) * 1.00),
                                2.00)
                        + 0.5);
            }
            // 所有放错位的数码的个数
        ...
        // 尝试一下学号大法
        jNode.setEstimatedValue((s + totalErrorNumber) * 12
                + totalDistance * (3 * 5)
                + jNode.getNodeDepth());
    }
}
